...
public class gazeEyes : MonoBehaviour
{
#pragma warning disable 649 // Field is never assigned
    ...
    [SerializeField, Tooltip("Left eye transform with x facing backward and z to the left.")] 
    private Transform leftEye;
    [SerializeField, Tooltip("Right eye transform with x facing backward and z to the left.")] 
...
#pragma warning restore 649
    ...
    {
        //_socialEyeData.Tick();
        var eyeTrackingData = Tobii.XR.TobiiXR.GetEyeTrackingData(Tobii.XR.TobiiXR_TrackingSpace.World);
        var gazeRay = eyeTrackingData.GazeRay;
        Vector3 worldGazePoint;
        if (gazeRay.IsValid)
        {
            Ray ray = new Ray(Camera.main.transform.position, gazeRay.Direction);
            if (false/*eyeTrackingData.ConvergenceDistanceIsValid*/)
            {
                worldGazePoint = ray.GetPoint(eyeTrackingData.ConvergenceDistance);
            }
            else
            {
                worldGazePoint = ray.GetPoint(maxDistance);
            }
        }
        else
        {
            worldGazePoint = Camera.main.transform.position + Camera.main.transform.forward * maxDistance;
        }

            
        // Rotate each eye to look at the world gaze point.
        leftEye.LookAt(worldGazePoint);
        leftEye.Rotate( new Vector3(180,90,0));
        rightEye.LookAt(worldGazePoint);
        rightEye.Rotate(new Vector3(180, 90, 0));
        ...
        UpdateBlinkBlendShape(eyeTrackingData.IsLeftEyeBlinking, ref _leftBlinkCurrentValue, BlendShape.LeftEyeLid);
        UpdateBlinkBlendShape(eyeTrackingData.IsRightEyeBlinking, ref _rightBlinkCurrentValue, BlendShape.RightEyeLid);
        ...
        UpdateLookingUpBlendShapes(worldGazePoint, verticalGazeAngle);
        UpdateLookingDownBlendShapes(worldGazePoint, verticalGazeAngle);
    }
